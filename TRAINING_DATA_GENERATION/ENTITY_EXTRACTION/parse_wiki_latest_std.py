...
def coRefResolution( curr_idx_, lines_, line_, nnp_D ):
    ...
    for wdidx, wd in enumerate( line_.split() ):
        ...
        if wd.lower() in [ 'his', 'her', 'she', 'he', 'these', 'this' ]:
            ...
            for kk in srtD_keys_:
              if kk < curr_idx_: # so if curr idx is 5 and reverse sorted key is 3, it implies presence of the person's name in htis line
                ...
                if len( nnp_list_ ) > 1:
                  ...
                  for nnp_ in nnp_list_:
                      print('MIKE->', nnp_, ref_line_, nnp_ in ref_line_ )
...
def extractEntity( title, paragraph, header ):
    ...
    for idx, line_ in enumerate( lines_ ):
        ...
        NNPs_ = nltk_nlp.returnOnlyProperNouns( str_ )
...
def findEntityRelation( title, dict_ ):
    ...
    for paragraph, header in dict_.items():
        if header != 'NA':
            imp_line, entity = extractEntity( title, paragraph, header )

            with open('RESULT.txt', 'a') as fp:
                fp.write( '---------------------------------\n' )
                fp.write( str( paragraph ) + '$$$ IMPORTANT ENTITY = ' + str( entity ) + '\n' ) 

            if imp_line is not None:
                print( 'IMP LINE->', imp_line )
                print( 'IMP ENTITY->', entity )
                print( 'PARA->', paragraph )
            else:
                print('NO AVAILABLE ENTITY -> merge with prior ?')
                print( paragraph )

        elif header == 'NA':
            imp_line, entity = extractEntityWithoutHeader( paragraph )

            if imp_line is not None:
                print( 'HEADERLESS IMP LINE->', imp_line )
                print( 'HEADERLESS IMP ENTITY->', entity )
                print( 'PARA->', paragraph )
            else:
                print('NO AVAILABLE ENTITY -> merge with prior ?')
                print( paragraph )
