...
class addAPIUsageToGraph():
    def __init__(self, src_dir_):

        self.src_dir_ = src_dir_
    ...
    def processResponse( self, url_assignments_, url_assignments_within_methods_, url_usages_, api_definitions_ ):
        ...
        for tup in api_definitions_:
            ...
            tmp_method_ll_ = self.method_summary_[ file_path ] if file_path in self.method_summary_ else []
            ...
            self.method_summary_[ file_path ] = tmp_method_ll_
        
        return self.method_summary_

    def createGraphInput(self):

        url_assignments_, url_assignments_within_methods_, url_usages_ = findAPIDefAndUsage( self.src_dir_ )
        ...
        api_definitions_ = find_api_definitions( self.src_dir_ )
        '''
        list of dicts
    ...
    def addToGraph(self):
        ...
        resp_ = self.createGraphInput()
        
        with open( 'examine.json', 'a+' ) as fp:
            json.dump( resp_, fp, indent=4 )

        ##NOTE-> TBD need to call graph entry from ../graph_utils
