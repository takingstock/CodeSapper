...
class generateGraph():
    ...
    def __init__( self, src_dir='', neo4j_config=None, dest_dir='' ):
    ...
    def createNodes( self, fnm, method_deets ):
        '''
        cypher query to create the node ..only if it doesnt exist
        '''
        ...
        ON CREATE SET m.method_begin_snippet = "''' + method_deets["method_begin"].replace('"', "'") + '''",
              m.method_end_snippet = "''' + method_deets["method_end"].replace('"', "'") + '''",
              m.method_begin_ln = "''' + str(method_deets["range"][0]) + '''",
              m.method_end_ln = "''' + str(method_deets["range"][1]) + '''",
              m.method_importance_ = 0.1
    ...
    def createRelations(self, relation_properties_):
        '''
        cypher query to create the relation ..only if it doesnt exist
        '''
        ...
        OPTIONAL MATCH (a)-[r:''' + relation_properties_["connection_type"] + ''' {
           code_snippet: "''' + relation_properties_["code_snippet"].replace('"', "'") + '''",
           relation_weight: 0.1
        }]->(b)
        WITH a, b, r
        WHERE r IS NULL
        CREATE (a)-[: '''+ relation_properties_["connection_type"]+''' {
            code_snippet: "'''+ relation_properties_["code_snippet"].replace('"',"'")+'''",
            relation_weight: 0.1
        }]->(b)
        '''
    ...
    def returnEdgeData( self, fnm, file_contents_, mode, idx ):
        ...
        tempD_["code_snippet"] = '\n'.join( usage_instance["usage"] ) \
                                   if type( usage_instance["usage"] ) is list \
                                   else usage_instance["usage"]
    ...
    def preProcess(self, method_summary_):
        '''
          ...
          for fc in file_contents_:
            ...
            if "global_uses" in fc:
                for idx, usage_ in enumerate( fc[ "global_uses" ] ):
                    ...
                    edge_insertion_cypher_ = self.createRelations( edge_data_ )
                    print('GLOBAL EDGE CYPHER->', edge_insertion_cypher_ )
                    edgeList_.append( edge_insertion_cypher_ )
    ...
    def execNodeCreationCypher(self, nodeList_, edgeList_ ):
        ...
        node_res, edge_res, err_obj_ = [], [], None
        with GraphDatabase.driver( self.NEO4J_URI, auth=self.NEO4J_AUTH ) as driver:
          try:
              with driver.session() as session:
                  ...
                  for node_insertion_cypher_ in nodeList_:
                    ...
                    node_res.append( result )

                  for edge_insertion_cypher_ in edgeList_:
                    #print('EDGE INSERTION QRY->', edge_insertion_cypher_)  
                    result = session.run( edge_insertion_cypher_ )
                    #print('EDGE INSERTION RES->', result)
                    edge_res.append( result )

          except:
              print('CYPHER INSERTION ERR->', traceback.format_exc())
              err_obj_ = traceback.format_exc()

        if len( node_res ) == len( nodeList_ ) and len( edge_res ) == len( edgeList_ ):
            return True, None
        else:
            return False, err_obj_
